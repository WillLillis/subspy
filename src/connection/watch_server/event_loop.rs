...
impl WatchServer {
    ...
    /// Resolves a git-side event to its submodule slot. An event under `.git/modules`
    /// without a current route indicates stale topology, so schedule a replacing
    /// reindex.
    fn route_submodule_git_event(
        &self,
        event: &notify::Event,
        structural_debounce: &mut ReindexDebounce,
    ) -> Option<usize> {
        let slot = self.submod_for_event(event);
        if slot.is_none() {
            structural_debounce.arm();
        }
        slot
    }
    ...
    fn handle_git_event(
        ...
        arm_reindex_on_rescan(event, tripwire_debounce);
        ...
        match self.classify_and_trace_git_event(event) {
            ...
            Some(EventType::SubmoduleGitOperation) => {
                if let Some(i) = self.route_submodule_git_event(event, tripwire_debounce) {
                    self.try_spawn_submod_update(i, in_flight, pending_status_retries);
                }
            }
...
/// Schedules a replacing reindex when the watcher reports that its event queue
/// overflowed. Dropped events cannot be reconstructed individually.
fn arm_reindex_on_rescan(event: &notify::Event, debounce: &mut ReindexDebounce) {
...
/// A lock release is a chance to retry a failed status read.

...
impl WatchServer {
    ...
    pub(super) fn watch_submodule(&mut self, watch_path: &Path) -> notify::Result<()> {
        let tree = self.tree_watch.as_mut().unwrap();
        match tree
            .watcher
            .watch(watch_path, notify::RecursiveMode::Recursive)
        {
            Err(error)
                if matches!(&error.kind, notify::ErrorKind::PathNotFound)
                    // TODO: See if notify will accept a change to its Windows backend to
                    // propagate `std::fs::metadata`'s `std::io::Error` via `notify::Error::io_watch`
                    // from `std::io::ErrorKind::NotFound` to `notify::ErrorKind::PathNotFound`.
                    || matches!(
                        &error.kind,
